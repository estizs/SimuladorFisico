...
public class Controller {
	...
	public void run(int n, OutputStream out, InputStream expOut, StateComparator cmp) throws NotEqualStatesException{
		...
		if(expOutJo != null) {
			...
			if(!cmp.equal(expState, currState)) throw new NotEqualStatesException(currState, expState, 0);
		}
		for (int i = 0; i < n; ++i) {
			simulator.advance();
			currState = simulator.getState();
			if(expOutJo != null) {
				...
				if(!cmp.equal(expState, currState)) throw new NotEqualStatesException(currState, expState, i);
			}
			p.println(currState);
		}
		p.println("]");
